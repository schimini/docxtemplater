...
describe("Pptx generation", function() {
	...
	it("should work with simple raw pptx async", function() {
		...
		createDoc("raw-xml-example.pptx")
			.then(doc => {
				...
				return doc.compile().then(() => doc);
			})
			.then(doc => {
				return doc.resolveData({ raw }).then(function() {
					return doc.render().then(() => doc.getFullText());
				})
					.then(fullText => {
						expect(calls).to.equal(1);
						expect(scope.raw).to.be.a("string");
	...
	it("should be possible to have linebreaks if setting the option", function(done) {
		createDoc("tag-multiline.pptx")
			.then(doc => {
				doc.setData({
					description: "The description,\nmultiline",
				});
				doc.setOptions({ linebreaks: true });
				return doc.render().then(() => doc);
			})
			.then(doc => {
	...
	it("should not fail when using linebreaks and tagvalue not a string", function(done) {
		createDoc("tag-multiline.pptx")
			.then(doc => {
				doc.setData({
					description: true,
...
describe("Table", function() {
	...
	it("should work with tables", function(done) {
		...
		createDoc("tag-intelligent-loop-table.docx")
			.then(doc => {
				doc.setData(tags);
				return doc.render().then(() => doc);
			})
			.then(doc => {
				...
				doc.getFullText().then(fullText => {
					expect(fullText).to.be.equal(expectedText);
					shouldBeSame({
						doc,
	...
	it("should work with simple table", function(done) {
		createDoc("table-complex2-example.docx")
			.then(doc => {
				doc.setData({
					table1: [
						{
	...
	it("should work with more complex table", function(done) {
		createDoc("table-complex-example.docx")
			.then(doc => {
				doc.setData({
					table2: [
						{
	...
	it("should not corrupt table with empty loop", function(done) {
		createDoc("table-loop.docx")
			.then(doc => {
				doc.setData({});
				doc.setOptions({ paragraphLoop: true });
				return doc.render(() => doc);
...
describe("Dash Loop Testing", function() {
	...
	it("dash loop ok on simple table -> w:table", function(done) {
		...
		createDoc("tag-dash-loop-table.docx")
			.then(doc => {
				doc.setData(tags);
				return doc.render().then(() => doc.getFullText());
			})
	...
	it("dash loop ok on simple list -> w:p", function(done) {
		...
		createDoc("tag-dash-loop-list.docx")
			.then(doc => {
				doc.setData(tags);
				return doc.render().then(() => doc.getFullText());
			})
...
describe("Templating", function() {
			...
			createDoc("tag-example.docx")
				...
				.then(doc => {
					return Promise.all([
						...
						doc.getFullText("word/header1.xml").then(fullText => {
							expect(fullText).to.be.equal(
	...
	it("should show spaces with linebreak option", function(done) {
		createDoc("tag-multiline.docx")
			.then(doc => {
				doc.setData({
	...
	it("should work with linebreaks without changing the style", function(done) {
		createDoc("multi-tags.docx")
			.then(doc => {
				doc.setData({
					test: "The tag1,\nmultiline\nfoobaz",
					test2: "The tag2,\nmultiline\nfoobar",
	...
	it("should work with paragraphloop", function(done) {
		createDoc("users.docx")
			.then(doc => {
				doc.setOptions({
					paragraphLoop: true,
				});
				return doc.setData({ users: ["John", "Jane", "Louis"] }).render().then(() => doc);
			})
			.then(doc => {
	...
	it("should work with paragraphloop without removing extra text", function(done) {
		createDoc("paragraph-loops.docx")
			.then(doc => {
				doc.setOptions({
					paragraphLoop: true,
				});
	...
	it("should fail properly when having lexed + postparsed errors", function(done) {
		createDoc("multi-errors.docx")
			.then(doc => {
				...
				return expectToThrowAsync(create, Errors.XTTemplateError, expectedError).then(() => {
					done();
				});
			});
	});
	...
	it("should work with spacing at the end", function(done) {
		createDoc("spacing-end.docx")
			.then(doc => {
				doc.setOptions({
					paragraphLoop: true,
				});
				return doc.setData({ name: "John" }).render().then(() => doc);
			})
			.then(doc => {
	...
	it("should work with custom properties", function(done) {
		createDoc("properties.docx")
			.then(doc => {
				return Promise.all([
					doc.getZip().files["docProps/app.xml"].async("string"),
					doc.getZip().files["docProps/core.xml"].async("string"),
					doc,
				]);
			})
			.then((lst) => {
...
describe("Prefixes", function() {
	it("should be possible to change the prefix of the loop module", function(done) {
		const content = "<w:t>{##tables}{user}{/tables}</w:t>";
		const scope = {
			tables: [{ user: "John" }, { user: "Jane" }],
		};
		createXmlTemplaterDocxNoRender(content, { tags: scope })
			.then(doc => {
			...
			.then(fullText => {
				expect(fullText).to.be.equal("JohnJane");
				done();
			});
	});

	it("should be possible to change the prefix of the loop module to a regexp", function(done) {
		const content =
		...
		createXmlTemplaterDocxNoRender(content, { tags: scope })
			...
			.then(fullText => {
				expect(fullText).to.be.equal("ABAB");
				done();
			});
	});

	it("should be possible to change the prefix of the raw xml module to a regexp", function(done) {
		...
		createXmlTemplaterDocxNoRender(content, { tags: scope })
			...
			.then(fullText => {
				expect(fullText).to.be.equal("HoHo");
				done();
			});
	});
});
...
describe("Load Office 365 file", function() {
	it("should handle files with word/document2.xml", function(done) {
		createDoc("office365.docx")
			...
			.then(doc => {
				doc.getFullText().then(fullText => {
					expect(fullText).to.be.equal("Value Value2");
...
describe("Resolver", function() {
	...
	it("should not regress 1 sync", function(done) {
		createDoc("regression-1.docx")
			.then(doc => doc.compile().then(() => doc))
			.then(doc => {
	...
	it("should not regress 1 async", function(done) {
		createDoc("regression-1.docx")
			.then(doc => doc.compile().then(() => doc))
			.then(doc => {
	...
	it("should not regress 2 sync", function(done) {
		createDoc("regression-2.docx")
			.then(doc => doc.compile().then(() => doc))
			.then(doc => {
	...
	it("should not regress 2 async", function(done) {
		createDoc("regression-2.docx")
			.then(doc => doc.compile().then(() => doc))
			.then(doc => {
				return doc.resolveData(regress2Data)
					.then(() => doc.render())
					.then(() => {
						shouldBeSame({ doc, expectedName: "expected-regression-2.docx" });
						done();
