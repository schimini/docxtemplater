...
describe("XmlTemplater", function() {
	it("should work with simpleContent", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
	...
	it("should work with {.} for this inside loop", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
				expect(fullText).to.be.equal("Hello Edgar,John,");
			});
	});
	...
	it("should work with non w:t content", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => getContent(xmlTemplater)))
			.then(c => {
    ...
    <w:p><w:r><w:t>{.}{/ab}</w:t></w:r></w:p>`;
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
				expect(fullText).to.be.equal("123");
			});
	});
	...
	it("should work with simple Loop", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
				expect(fullText).to.be.equal("Hello Edgar,Mary,John,");
			});
	});
	...
	it("should work with simple Loop with boolean value falsy", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
				expect(fullText).to.be.equal("Hello ");
			});
	});
	...
	it("should work when replacing with exact same value", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()
				.then(() => xmlTemplater.getFullText())))
			.then(fullText => {
				expect(fullText).to.be.equal("Hello {name}");
			});
	});
...
describe("Change the nullGetter", function() {
	it("should work with null", function() {
		...
		createXmlTemplaterDocxNoRender(content, {
			...
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
				expect(fullText).to.be.equal("Hello null");
			});
	});
	...
	it("should work with null in resolve", function() {
		...
		createXmlTemplaterDocxNoRender(content, {
			nullGetter,
		}).then(xmlTemplater => xmlTemplater.compile(() => xmlTemplater))
			.then(xmlTemplater => {
				return xmlTemplater.resolveData(data).then(function() {
					expect(calls).to.be.equal(1);
					return xmlTemplater.render().then(() => xmlTemplater)
						.then(xmlTemplater => xmlTemplater.getFullText())
						.then(fullText => {
							expect(calls).to.be.equal(1);
							expect(fullText).to.be.equal("Hello null");
						});
...
describe("Custom delimiters", function() {
	...
	it("should work with custom delimiters with two chars", function() {
		...
		createXmlTemplaterDocx(content, {
			...
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater.getFullText()))
			.then(fullText => {
				expect(fullText).to.be.eql("Hello Edgar");
			});
	});
	...
	it("should work with loops", function() {
		...
		createXmlTemplaterDocx(content, {
			...
			.then(xmlt => xmlt.render().then(() => getContent(xmlt)))
			.then(c => {
				expect(c).to.be.equal(
