...
describe("Docxtemplater loops", function() {
	...
	it("should work with object value", function() {
		...
		createXmlTemplaterDocx(content, { tags: scope }).then(xmlTemplater => {
			return xmlTemplater.render().then(() => {
				return getContent(xmlTemplater);
			});
		}).then(c => {
...
describe("Changing the parser", function() {
	it("should work with uppercassing", function() {
		...
		createXmlTemplaterDocx(content, {
			...
			xmlTemplater.render().then(() => {
				return xmlTemplater.getFullText();
			}).then(fullText => {
	...
	it("should work when setting from the Docxtemplater interface", function() {
		createDoc("tag-example.docx").then(doc => {
			JSZip.loadAsync(doc.loadedContent).then((zip) => {
				...
				d.render().then(() => {
					d.getFullText().then((fullText) => {
						expect(fullText).to.be.equal("EDGAR HIPP");
					});
				}).then(() => {
					d.getFullText("word/header1.xml").then((fullText) => {
						expect(fullText).to.be.equal(
							"EDGAR HIPP0652455478NEW WEBSITE"
						);
					});
				}).then(() => {
					d.getFullText("word/footer1.xml").then((fullText) => {
						expect(fullText).to.be.equal(
							"EDGARHIPP0652455478"
						);
					});
	...
	it("should work with angular parser", function() {
		...
		createDoc("angular-example.docx").then(doc => {
			...
			doc.render().then(() => {
				...
				expect(fullText).to.be.equal("Hipp Edgar 2014");
			});
		});
	});
	...
	it("should work with loops", function() {
		...
		createXmlTemplaterDocx(content, {
			...
			xmlTemplater.render().then(() => {
				...
				expect(fullText).to.be.equal("Hello you");
			});
		});
	});
	...
	it("should be able to access meta to get the index", function() {
		...
		createXmlTemplaterDocx(content, {
			...
			xmlTemplater.render().then(() => {
				...
				expect(fullText).to.be.equal("Hello 0 Jane 1 Mary ");
			});
		});
	});
	...
	it("should be able to access meta to get the type of tag", function() {
		...
		createXmlTemplaterDocx(content, {
			...
			expect(fullText).to.be.equal("Hello Jane");
...
describe("Change the delimiters", function() {
	it("should work with lt and gt delimiter < and >", function() {
		createDoc("delimiter-gt.docx").then(doc => {
			...
			doc.render().then(() => doc.getFullText()).then(fullText => {
...
describe("Special characters", function() {
	...
	it("should render placeholder containing special characters", function() {
		const content = "<w:t>Hello {name}</w:t>";
		const scope = { name: "<Edgar>" };
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => getContent(xmlTemplater))
			.then(c => {
	...
	it("should read full text correctly", function() {
		createDoc("cyrillic.docx")
			.then(doc => doc.getFullText())
			.then(fullText => {
	...
	it("should insert russian characters", function() {
		...
		createDoc("tag-example.docx")
			.then(doc => JSZip.loadAsync(doc.loadedContent))
			.then(zip => new Docxtemplater().loadZip(zip))
			.then(d => {
				d.setData({ last_name: russian });
				return d.render().then(() => d.getFullText());
			})
			.then(outputText => {
...
<w:t>{/tables}</w:t>`,
			...
			createXmlTemplaterDocx(content, { tags: scope })
				.then(doc => doc.render().then(() => getContent(doc)))
				.then(c => {
...
<w:t/>`
	...
	it("should work when looping inside tables", function() {
		...
		<w:t>{key}</w:t>
		`;
		createXmlTemplaterDocx(template, { tags })
			.then(doc => {
				doc.render().then(() => {
					doc.getFullText()
						.then(fullText => {
...
describe("Raw Xml Insertion", function() {
	...
	it("should work with closing tag in the form of <w:t>}{/body}</w:t>", function() {
		...
		<w:t>{paragraph</w:t>
			<w:t>}{/body}</w:t>`;
		createXmlTemplaterDocx(content, { tags: scope })
			.then(xmlTemplater => xmlTemplater.render().then(() => xmlTemplater))
			.then(xmlTemplater => getContent(xmlTemplater))
			.then(c => {
	...
	it("should work with simple example and given options", function() {
		...
		createDoc("one-raw-xml-tag.docx")
			.then(doc => {
				doc.setOptions({
			...
			.then(doc => doc.getFullText())
			.then(fullText => {
				expect(fullText).to.be.equal("asdfMy customXMLqwery");
			});
	});
});
